...
if __name__ == "__main__":
    ...
    count = 0
    sess = tf.Session()
    m1_1 = Model_Adam(sess, "m11", learning_rate1)
    m2_1 = Model_Grad(sess, "m21", learning_rate1)
    m1_2 = Model_Adam(sess, "m12", learning_rate2)
    m2_2 = Model_Grad(sess, "m22", learning_rate2)
    m1_3 = Model_Adam(sess, "m13", learning_rate3)
    m2_3 = Model_Grad(sess, "m23", learning_rate3)
    ...
    for i in range(epoch):  # 10 번 반복
        ...
        for j in range(maxBatch):  # 200번 반복
            x_batch = train_x_data_03[startNumber:startNumber + 100]
            y_batch = train_y_data_03_onehot[startNumber:startNumber + 100]
            if (len(x_batch) != 0):
