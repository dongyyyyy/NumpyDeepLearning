...
if __name__ == "__main__":
    ...
    W2 = np.random.rand(8, 16)
    W3 = np.random.rand(16, 8)
    # print(W1)
    v_relu = V_ReLU()
    v_relu_back = V_ReLU_back()
    z1 = np.random.rand(batch, 8)
    h1 = np.random.rand(batch,8)

    z2 = np.random.rand(batch, 16)
    h2 = np.random.rand(batch, 16)

    z3 = np.random.rand(batch, 8)
    ...
    def forward(x, y):
        ...
        o = ODivideFunction(np.exp(z3), np.sum(np.exp(z3), axis=1))
    ...
    def backward(x, y, input_data):
        local_param3 = (x-y) # 100, 8
        func3 = v_relu_back(z3) # 100 8
        #local_param4 = func4 * local_param4 # 100 8
        result = np.dot(np.transpose(h2),local_param3) # 24 100 , 100 8
        delta_o = (-learning_rate)*result # 24 8
        NW3 = W3 + delta_o

        local_param2 = np.dot(local_param3, np.transpose(W3))# 100 8 8 24 -> 100 24
        func2 = v_relu_back(z2) # 100 24
        #local_param2 = local_param2 * func2 # 100 24
        result = np.dot(np.transpose(h1),local_param2) # 4 100 100 24
        delta_h2 = (-learning_rate) * result  # W1 변화량
        NW2 = W2 + delta_h2  # 다음 W1 = 현재 W1 + W1의 변화량
    ...
    for i in range(epoch):  # 10 번 반복
        ...
        for j in range(maxBatch):  # 200번 반복
            ...
            if (len(x_batch) != 0):
                ...
                W3, W2, W1 = backward(error, y_batch,x_batch)
                #print("W1[0]:",W1[0])
                #print("W2[0]:", W2[0])
